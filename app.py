...
# -----------------------------
def classify_document(text, slider_threshold):
    ...
    # Use slider threshold instead of JSON threshold
    ...
    if len(sorted_scores) >= 2:
        if sorted_scores[1][1] / sorted_scores[0][1] > 0.7:
            is_other = True
            other_reason = (
                f"Ambiguous - similar scores: {sorted_scores[0][0]} vs {sorted_scores[1][0]}"
            )
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Version 2.3 • PDF Support + Working Slider")
    ...
    if st.button("🔍 Classify Document", use_container_width=True):
        ...
        with st.expander("📊 Detailed Analysis", expanded=True):
            ...
            for category, score in sorted_scores:
                ...
                bar_width = score / max_score
                icon = CATEGORIES.get(category, {}).get("icon", "📄")
                st.write(f"{icon} **{category}**: {score}")
